...
export class ReasonsSectionApiService {
   ...
   */
  private loadFromJsonFile(): Observable<SimpleReasonsConfig> {
    const configUrl = './home-reasons-section.json';
    
    console.log('📥 Loading reasons config from JSON file:', configUrl);
    
    return this.http.get<any>(configUrl, this.httpOptions)
      .pipe(
        map((data: any) => {
          console.log('✅ Reasons config loaded successfully from JSON:', data);
          return this.mapApiConfigToSimpleConfig(data);
        }),
        catchError(error => {
          console.error(`❌ Failed to load reasons config from JSON:`, error);
